...
static void *client_handler(void *fd)
{
	...
	err = connect(ks->client, (struct sockaddr *)&client_in, sizeof(client_in));
	if (err < 0) {
...
static void *server_handler(void *fd)
{
	...
	err = setsockopt(ks->server,  SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT, &one, sizeof(one));
	if (err) {
		perror("setosckopt RESUSE failed\n");
		return NULL;
	}
	...
	err = bind(ks->server, (struct sockaddr *)&server_in, sizeof(server_in));
	if (err < 0) {
		perror("bind error\n");
		return NULL;
	}
	...
	ks->accept = accept(ks->server, (struct sockaddr *)&client_in, (socklen_t*)&_c);
	if (ks->accept < 0) {
		printf("accept error\n");
		return NULL;
	}
...
int main(int argc, char **argv)
{
	...
	while ((backend_client.client == 0 || frontend_client.client == 0) && running)
		sleep(1);
