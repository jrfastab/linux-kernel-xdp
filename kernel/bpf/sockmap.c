 ...
 * ./samples/bpf/sockmap/. Additionally, reviewing the test cases in
 * selftests/bpf may also provide hints on usage patterns.
 */
#include <linux/bpf.h>
...
static void smap_state_change(struct sock *sk)
{
	 ...
	 * being torn down so tear down the smap socket. strparser will only
	 * push code into verdict logic after connection is established.
	 */
	switch (sk->sk_state) {
	case TCP_SYN_RECV:
	...
	case TCP_CLOSE:
		 ...
		 * the psock is detached. The sk_callback_lock ensures we
		 * get valid user_data and the rcu critical section ensures
		 * we don't free psock before handling state change.
		 */
...
static void smap_schedule_writer(struct smap_psock *psock)
{
...
static void smap_read_sock_strparser(struct strparser *strp,
				     struct sk_buff *skb)
{
	struct smap_psock *psock = container_of(strp, struct smap_psock, strp);
...
void sock_map_remove_complete(struct bpf_stab *stab)
{
...
static void sock_map_free(struct bpf_map *map)
{
	 ...
	 * However, be aware we can still get a socket state event and call
	 * smap_release_sock() from that path. Also psock worker threads are
	 * still in-flight. By using an xchg operation on sock_map either we
	 * get a NULL here or a NULL in the sock event handler routine avoiding
	 * duplicate frees.
	 */
 ...
 * operations to ensure we do not get double free's. The psock is destroyed via
 * call_rcu so we are certain all references from the update/lookup/delete
 * operations are no longer in use.
 *
 * A psock object holds a refcnt on the sockmap it is attached on and this is
 ...
 *
 * The psock lock is used in the update case to ensure concurrent updates do
 * not try to simultaneously enable and disable the strparser feature. Setup
 * of strparser is not atomic. The lock is only needed in the update case for
 * _new_ sockets there can be only a single reference.
 *
 * A xchg operation populates the sockmap and the old_psock attached to that
 * sock, if any, will be destroyed.
 *
				...
				void *key, u64 flags, u64 map_flags)
{
	...
	/* reserve programs early so can abort easily on failures */
