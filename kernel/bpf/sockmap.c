 ...
 * parse program. If adding a sock object to a map would result in having
 * multiple parsing programs the update will return an EBUSY error.
 *
...
static void smap_release_sock(struct sock *sock, struct bpf_stab *stab);
...
static void smap_state_change(struct sock *sk)
{
	...
	case TCP_CLOSE:
		...
		list_for_each_entry_safe(e, tmp, &psock->maps, list) {
			osk = cmpxchg(&e->stab->sock_map[e->key], sk, NULL);
			if (osk == sk) {
				list_del(&e->list);
				smap_release_sock(sk, e->stab);
			}
		}
...
void smap_list_remove(struct sock *sk, u32 i, struct bpf_stab *stab)
{
	struct smap_psock *psock = smap_psock_sk(sk);
	struct smap_psock_map_entry *e, *tmp;

	list_for_each_entry_safe(e, tmp, &psock->maps, list) {
		if (e->key == i && e->stab == stab) {
			list_del(&e->list);
			break;
		}
	}
...
static void sock_map_free(struct bpf_map *map)
{
	...
	for (i = 0; i < stab->map.max_entries; i++) {
		...
		smap_list_remove(sock, i, stab);
		smap_release_sock(sock, stab);
 ...
 *  - sk_user_data must only be modified inside sk_callback_lock
				    ...
				    void *key, u64 flags)
{
	 ...
	 * which parser is running. If no psock exists create one.
	 */
	if (psock) {
		...
		if (!refcount_inc_not_zero(&stab->refcnt)) {
			err = -EINVAL;
			goto out_progs;
		}
...
start_done:
	...
	if (!update) {
		...
		if (old_sock) {
			if ((old_sock != sock) && parse)
				smap_stop_sock(psock, old_sock);
			smap_list_remove(old_sock, i, stab);
			smap_release_sock(old_sock, stab);
		}
	}
