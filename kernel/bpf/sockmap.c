...
static int smap_reserve_progs(struct bpf_stab *stab)
{
...
static void smap_init_progs(struct smap_psock *psock, struct bpf_stab *stab)
{
	...
	psock->bpf_verdict = stab->bpf_verdict;
...
static void sock_map_free(struct bpf_map *map)
{
	 ...
	 * smap_release_sock() from that path. By using an xchg operation on
	 * sock_map either we get a NULL here or a NULL in the sock event
	 * handler routine avoiding duplicate frees.
	 */
 ...
 * will not be released via smap_release_sock() when concurrent updates/deletes
 * try to remove the psock. The psock is destroyed via call_rcu so we are
 * certain all references from the update are no longer in use.
 *
 * We hold a refcnt on the sock that is not decremented until after a RCU
 * grace period. This ensures the sock itself is not released.
 *
				...
				void *key, u64 flags, u64 map_flags)
{
	...
	if (!psock) {
		...
		if (IS_ERR(psock)) {
			bpf_prog_sub(stab->bpf_verdict, 1);
			bpf_prog_sub(stab->bpf_parse, 1);
