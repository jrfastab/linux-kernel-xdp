...
static int smap_recv(struct smap_psock *psock, struct sk_buff *skb,
				     ...
				     struct sk_buff *skb)
{
	 ...
	 * sense.
	 */
	err = smap_recv(peer, skb, 0, skb->len);
	if (err)
		goto out;
...
start:
		do {
			n = skb_send_sock(psock->sock, skb, sent, skb->len);
			if (n <= 0) {
				if (n == -EAGAIN) {
...
{
	/* TBD need prog_put and gets here to avoid programs leaving us or something in attach */
...
static struct smap_psock *smap_init_psock(struct sock *sock, struct bpf_stab *stab)
{
	struct smap_psock *psock;
...
static struct bpf_map *sock_map_alloc(union bpf_attr *attr)
{
	...
	cost = (u64) stab->map.max_entries * sizeof(struct sock *) + sizeof(struct socket *);
...
static int sock_map_attach_prog(struct bpf_map *map, struct bpf_prog *parse, struct bpf_prog *mux)
{
	struct bpf_stab *stab = container_of(map, struct bpf_stab, map);
